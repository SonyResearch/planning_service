...
Toppra::Toppra(const Trajectory<double>& path,
               ...
               const motion::HolonomicMapping& holonomic_mapping)
    : backward_prog_ {new solvers::MathematicalProgram()},
      ...
      backward_cost_ {backward_prog_->AddLinearCost(
          Eigen::Vector2d::Zero(), 0, {backward_x_, backward_u_})},
  ...
  DRAKE_THROW_UNLESS(plant_.num_positions() == holonomic_mapping_.full_dim());
  DRAKE_THROW_UNLESS(path.rows() == holonomic_mapping_.minimal_dim());
...
Toppra::AddJointTorqueLimit(
  ...
  const int N = gridpoints_.size() - 1;
  const int n_dof = path_.rows();
  const int n_plant = plant_.num_positions();
  DRAKE_DEMAND(lower_limit.size() == n_dof);
  DRAKE_DEMAND(upper_limit.size() == n_dof);
  // Lift upper and lower limits to full space if necessary.
  const int n_con = discretization == ToppraDiscretization::kInterpolation
                        ? 2 * n_plant
                        : n_plant;
  Eigen::MatrixXd con_A(n_con, 2 * N);
  Eigen::MatrixXd con_lb(n_con, N);
  Eigen::MatrixXd con_ub(n_con, N);
  Eigen::MatrixXd M(n_plant, n_plant);
  Eigen::VectorXd Cv(n_plant);
  Eigen::VectorXd G(n_plant);
  std::vector<int> mimic_indices = holonomic_mapping_.MimickingJointIndices();
  for (int knot = 0; knot < N; knot++) {
    ...
    const Eigen::VectorXd qs_dot = path_.EvalDerivative(gridpoints_(knot), 1);
    const Eigen::VectorXd qs_ddot = path_.EvalDerivative(gridpoints_(knot), 2);
    const auto qs_lifted = holonomic_mapping_.Lift(qs);
    const auto qs_dot_lifted = holonomic_mapping_.Lift(qs_dot);
    const auto qs_ddot_lifted = holonomic_mapping_.Lift(qs_ddot);
    plant_.SetPositions(plant_context_.get(), qs_lifted);
    plant_.SetVelocities(plant_context_.get(), qs_dot_lifted);
    ...
    con_A.block(0, 2 * knot, n_plant, 1) << M * qs_ddot_lifted + Cv;
    con_A.block(0, 2 * knot + 1, n_plant, 1) << M * qs_dot_lifted;
    con_lb.block(0, knot, n_plant, 1)
        << holonomic_mapping_.Lift(lower_limit) + G;
    con_ub.block(0, knot, n_plant, 1)
        << holonomic_mapping_.Lift(upper_limit) + G;
    // Remove the constraint on the mimicing joints if any.
    for (const int mimic_idx : mimic_indices) {
      con_A(mimic_idx, 2 * knot) = 0;
      con_A(mimic_idx, 2 * knot + 1) = 0;
      con_lb(mimic_idx, knot) = -std::numeric_limits<double>::infinity();
      con_ub(mimic_idx, knot) = std::numeric_limits<double>::infinity();
    }
  }
  if (discretization == ToppraDiscretization::kInterpolation) {
    CalcInterpolationConstraint(&con_A, &con_lb, &con_ub);
...
Binding<BoundingBoxConstraint> Toppra::AddFrameVelocityLimit(
  ...
  for (int knot = 0; knot < N; knot++) {
    ...
    plant_.SetPositions(plant_context_.get(), holonomic_mapping_.Lift(qs));
    plant_.SetVelocities(plant_context_.get(), holonomic_mapping_.Lift(qs_dot));
...
Toppra::AddFrameAccelerationLimit(const Frame<double>& constraint_frame,
  ...
  for (int knot = 0; knot < N; knot++) {
    ...
    con_A.block(0, 2 * knot, 6, 1)
        << J * holonomic_mapping_.Lift(qs_ddot) + dJds_times_v;
    con_A.block(0, 2 * knot + 1, 6, 1) << J * holonomic_mapping_.Lift(qs_dot);
...
std::optional<Eigen::Matrix2Xd> Toppra::ComputeBackwardPass(
  ...
  my_logging::log()->info(fmt::format(
      "ComputeBackwardPass: s_dot_0: {}, s_dot_N: {}", s_dot_0, s_dot_N));
  ...
  for (int knot = N - 1; knot > -1; knot--) {
    ...
    for (auto& [constraint, bounds] : x_bounds_) {
      double lower_bound = bounds.lb(knot);
      double upper_bound = bounds.ub(knot);
      constraint.evaluator()->set_bounds(
          Eigen::VectorXd::Constant(1, lower_bound),
  ...
  Eigen::MatrixXd Table(gridpoints_.size(), 5);
  for (int i = 0; i < gridpoints_.rows(); i++) {
    const double t = gridpoints_(i);
    const Eigen::VectorXd qs = path_.value(t);
    const Eigen::VectorXd qs_dot = path_.EvalDerivative(t, 1);
    const Eigen::VectorXd qs_ddot = path_.EvalDerivative(t, 2);
    Table(i, 0) = t;
    // Get the infinity norm of the velocity and acceleration
    Table(i, 1) = qs_dot.lpNorm<Eigen::Infinity>();
    Table(i, 2) = qs_ddot.lpNorm<Eigen::Infinity>();
    Table(i, 3) = K(0, i);
    Table(i, 4) = K(1, i);
  }
  // Check problem is controllable
  const double x_start = std::pow(s_dot_0, 2);
  const double eps = 5e-2;  // @sadra: I changed this from 1e-8 to 5e-2
  // TODO: This is a hack to avoid numerical issues. We should find a better
  // way to handle this.
  my_logging::log()->info(
      "Toppra: Initial velocity: {} vs controllable set: [{}, {}].", x_start,
      K(0, 0), K(1, 0));
  if (x_start < K(0, 0) - eps || x_start > K(1, 0) + eps) {
    my_logging::log()->info(
        "Toppra: Backward pass completed. Your table is: \n{}", Table);
    my_logging::log()->error(
...
Toppra::ComputeForwardPass(double s_dot_0,
  ...
  for (int knot = 0; knot < N; knot++) {
    ...
    double min_u = (K(0, knot + 1) - xstar(knot)) / (2 * delta);
    double max_u = (K(1, knot + 1) - xstar(knot)) / (2 * delta);
    for (auto& [constraint, coefficients] : forward_lin_constraint_) {
      constraint.evaluator()->UpdateCoefficients(
          ...
          coefficients.ub.col(knot)
              - xstar(knot) * coefficients.coeffs.col(2 * knot));
      int n = coefficients.coeffs.rows();
      for (int i = 0; i < n; i++) {
        double coeff = coefficients.coeffs.col(2 * knot + 1)(i);
        double lb = coefficients.lb.col(knot)(i)
                    - xstar(knot) * coefficients.coeffs.col(2 * knot)(i);
        double ub = coefficients.ub.col(knot)(i)
                    - xstar(knot) * coefficients.coeffs.col(2 * knot)(i);
        DRAKE_DEMAND(lb <= ub + 1e-5);
        if (std::abs(coeff) < 1e-5) {
          continue;
        }
        if (coeff > 0) {
          max_u = std::min(max_u, ub / coeff);
          min_u = std::max(min_u, lb / coeff);
        } else {
          max_u = std::min(max_u, lb / coeff);
          min_u = std::max(min_u, ub / coeff);
        }
      }
    }
    if (min_u > max_u + 1e-4) {
      my_logging::log()->error(fmt::format(
          "Toppra: Infeasible Forward pass: knot: {}/{}, min_u: {}, max_u: {}",
          knot, N, min_u, max_u));
      return std::nullopt;
    }
    ustar(knot) = scale * max_u + (1 - scale) * min_u;
    double xnext = xstar(knot) + 2 * delta * ustar(knot);
    xstar(knot + 1) = std::max(K(0, knot + 1), std::min(K(1, knot + 1), xnext));
    DRAKE_DEMAND(std::abs(xstar(knot + 1) - xnext) < 2e-4);
  }
  my_logging::log()->info("Toppra: Forward pass completed in {} ms.",
...
std::optional<PiecewisePolynomial<double>> Toppra::SolvePathParameterization(
  ...
  // Start from scale 1.00 and gradually decrease to 0.9 by 0.01
  // ToDo (@sadraddini): This is a hack to avoid numerical issues. And this
  // works. The numbers must be provided by the user.
  std::optional<std::pair<Eigen::VectorXd, Eigen::VectorXd>> forward_results;
  for (double scale = 1.00; scale > 0.9; scale -= 0.01) {
    forward_results = ComputeForwardPass(s_dot_start, K.value(), scale);
    if (forward_results) {
      my_logging::log()->info(
          "Toppra: Found a valid forward pass with scale: {}", scale);
      break;
    }
  }
  if (!forward_results
           .has_value()) {  // Error occurred in forward pass, return nothing
    my_logging::log()->error(
        "Toppra failed to find a valid forward pass after "
        "trying multiple scales.");
    return std::nullopt;
  }
  ...
  if (sd_knots.hasNaN()) {
    my_logging::log()->error("Toppra hit numerical issues. Found NaN sdot.");
    return std::nullopt;
  }

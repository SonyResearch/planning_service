...
int main(int argc, char** argv) {
  ...
  for (const auto& problem : problems) {
    logging::log()->info(
        "BuildArtifactsFromProblem: Building artifacts using problem with "
        "start {} and goal {}",
    ...
    planner.mutable_artifact_builder().SolveProblemAndBuildRegions(
        problem.q_start, problem.q_goal,
        motion::iris::IrisBuilder::IrisMethod::kIrisNp2, false);
  }
}
